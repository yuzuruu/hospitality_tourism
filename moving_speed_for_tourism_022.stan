...
parameters {
  real<lower=0> mu_r_mode;
  real<lower=0> mu_r_occasion;
...
model {
  ...
  s_v ~ normal(0, 1);
  mu_r_mode ~ normal(0, 1);
s_h_mode ~ normal(0, 1);
s_r_mode ~ student_t(3, 0, 10);
  mu_r_occasion ~ normal(0, 1);
s_h_occasion ~ normal(0, 1);
s_r_occasion ~ student_t(3, 0, 10);
  ...
  r_raw_mode ~ normal(mu_r_mode, s_h_mode);
  r_raw_occasion ~ normal(mu_r_occasion, s_h_occasion);
